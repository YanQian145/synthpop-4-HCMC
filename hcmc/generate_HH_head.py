...
def initialization(
    ...
    merged_census_HH[district] = pd.merge(
        census_HH[district],
        HH_level_table[district][ward][["HH_type", "proba"]],
        how="left",
        left_on=["HH_type"],
        right_on=["HH_type"],
    )
...
def pick_and_check_HH_desirable(
    ...
    random_draw = merged_census_HH[district].sample(
        n=1, weights=merged_census_HH[district]["proba"]
    )
    ...
    counter_match_pop = np.where(
        df2["tot_POPI"] + df2["count"]
        <= df2["tot_POP"] + df2["tot_POP"] * (percent / 100),
        0,
        1,
    )
...
def update_counters(
    ...
    _dict_counts_per_gender_age = pd.DataFrame.from_dict(
        Counter(ID_POP_type_list), orient="index"
    ).reset_index()
    _dict_counts_per_gender_age = _dict_counts_per_gender_age.rename(
        columns={"index": "ID_match_ag", 0: "count"}
    )
    # transform into dataframe, and merge with POP dd to sum together counts
    _temp_dict_POP = pd.merge(
        POP_level_table[district][ward],
        _dict_counts_per_gender_age,
        how="left",
        left_on=["ID_match_ag"],
        right_on=["ID_match_ag"],
    )
...
def generate_HH_head(simulation, district):
    ...
    for ward, df in dict_HH[district].items():
        ...
        print(
            f"start district {district} , ward {ward} with x HH: {dict_HH[district][ward]['tot_HH'].sum()} simulation {str(simulation)}"
        )
        ...
        dict_HH, dict_POP, merged_census_HH_proba = initialization(
            district,
            ward,
            merged_census_HH=merged_census_HH_proba,
            census_HH=dict_df_census_HH,
            HH_level_table=dict_HH,
        ...
        while (merged_census_HH_proba[district]["proba"].sum() > 0) & (
            ...
            while continue_looking:
                continue_looking, desirable_HH, i_max = pick_and_check_HH_desirable(
                    i_max=i_max,
                    district=district,
                    ward=ward,
                    HH_level_table=dict_HH,
            ...
            (
                ...
                POP_level_table=dict_POP,
                census_HH=dict_df_census_HH,
                synthetic_population=synth_pop_list,
                random_draw=desirable_HH,
        ...
        text = f"generated HH for district {district} , ward {ward} with x HH: {dict_HH[district][ward]['tot_HH'].sum()} simulation {str(simulation)} - total_ {diff}"
    ...
    # synth pop
    pkl_file = os.path.join(out_folder, f"synthHH_dis{district}.pkl")
